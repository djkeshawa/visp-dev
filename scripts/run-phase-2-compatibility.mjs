...
function parseArguments(argv) {
  ...
  for (const field of [
    ...
    if (!input[field]) throw new TypeError(`Missing required Phase 2 argument: ${field}`);
  }
...
try {
  ...
  if (parsed.mode === "verify") {
    ...
    report = await runPackedPhase2Compatibility(parsed.input);
  ...
  if (report.retainedRoot) {
    process.stderr.write(`phase-2-compatibility: retained root ${report.retainedRoot}\n`);
  }
} catch (error) {
  process.stderr.write(`phase-2-compatibility: ${error.message}\n`);
  if (error.retainedRoot) {
    process.stderr.write(`phase-2-compatibility: retained root ${error.retainedRoot}\n`);
  }
  process.stdout.write(canonicalStringify({
    error: { code: error.code ?? error.name ?? "PHASE_2_COMPATIBILITY_ERROR" },
    schemaVersion: "visp.phase-2-compatibility.error.v1",
  }));
  process.exitCode = 1;
}
