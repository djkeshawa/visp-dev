...
function parseArguments(argv) {
  ...
  for (const field of [
    ...
    if (!input[field]) throw new TypeError(`Missing required Phase 4 argument: ${field}`);
  }
...
try {
  ...
  if (parsed.mode === "verify") {
    ...
    report = await runPackedPhase4Compatibility(parsed.input);
  ...
  if (report.retainedRoot) {
    process.stderr.write(`phase-4-compatibility: retained root ${report.retainedRoot}\n`);
  }
} catch (error) {
  process.stderr.write(`phase-4-compatibility: ${error.message}\n`);
  if (error.retainedRoot) {
    process.stderr.write(`phase-4-compatibility: retained root ${error.retainedRoot}\n`);
  }
  process.exitCode = 1;
}
